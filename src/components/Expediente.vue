<template>
  <v-card class="mx-auto my-6" elevation="16">
    ...
    <v-card-text>
      <v-row dense>
        ...
        <v-col cols="6" md="6">
          <v-text-field
            ...
            v-maska="'##:##'"
            maxLength="5"
            v-model="expediente.hora_abertura"
            type="time"
            @blur="
              expediente.hora_abertura = formatarHorario(
                expediente.hora_abertura
              )
            "
            id="hora_abertura"
            name="hora_abertura"
        ...
        <v-col cols="6" md="6">
          <v-text-field
            ...
            v-maska="'##:##'"
            maxLength="5"
            v-model="expediente.hora_fechamento"
            type="time"
            @blur="
              expediente.hora_fechamento = formatarHorario(
                expediente.hora_fechamento
              )
            "
            id="hora_fechamento"
        ...
        <v-col cols="6" md="6">
          <v-text-field
            ...
            v-maska="'##:##'"
            maxLength="5"
            v-model="expediente.intervalo_inicio"
            type="time"
            @blur="
              expediente.intervalo_inicio = formatarHorario(
                expediente.intervalo_inicio
              )
            "
            id="intervalo_inicio"
        ...
        <v-col cols="6" md="6">
          <v-text-field
            density="compact"
            :rules="[rules.required]"
            label="TÉRMINO INTERVALO"
            v-maska="'##:##'"
            maxLength="5"
            v-model="expediente.intervalo_fim"
            type="time"
            @blur="
              expediente.intervalo_fim = formatarHorario(
                expediente.intervalo_fim
              )
            "
            id="intervalo_fim"
            name="intervalo_fim"
      ...
      <v-list>
        <v-card
          v-for="(item, index) in listaExpedientes"
          :key="index"
          class="mb-4"
        >
          <v-list-item
          ...
          >
            <template v-slot:prepend>
              ...
              <strong class="text-caption">{{
                getDiaDescricao(item.horario_expediente_id)
              }}</strong>
            </template>
            <template v-slot:append>
              <div class="d-flex justify-space-between horario-wrapper">
                <span
                  ...
                  >{{ formataHora(item.hora_abertura) }} /</span
                >
                <span
                  class="font-weight-black text-body-2 mt-1 horario text-caption"
                  >{{ formataHora(item.intervalo_inicio) }} /</span
                >
                <span
                  class="font-weight-black text-body-2 mt-1 horario text-caption"
                  >{{ formataHora(item.intervalo_fim) }} /</span
                >
                <span
                  class="font-weight-black text-body-2 mt-1 horario text-caption"
                  >{{ formataHora(item.hora_fechamento) }}
                </span>
              </div>
            </template>
          </v-list-item>
...
export default {
  ...
  methods: {
        ...
        if (credencial) {
          try {
            ...
            if (empresa && Object.keys(empresa).length > 0) {
              this.listaExpedientes = empresa.agenda_empresa_expedientes?.map(
                (item) => ({
                  id: item.id,
      ...
      if (this.empresa?.id) {
        ...
        if (credencial) {
          try {
            ...
            this.$store.dispatch("toast/showToast", {
              message: `Erro ao carregar dias da semana: ${error}`,
              color: "error",
            });
          } finally {
            this.$store.dispatch("loading/hideLoading");
    ...
    salvarExpediente() {
      ...
      this.$store
        .dispatch("expediente/salvarExpediente", payload)
        .then(() => {
          this.$store.dispatch("toast/showToast", {
        ...
        .catch((error) => {
          console.error("Erro ao salvar o expediente:", error);
          this.$store.dispatch("toast/showToast", {
            message: `Erro ao salvar o expediente`,
            color: "error",
          });
          this.loading = false;
        });
      ...
      if (this.empresa?.id) {
        ...
        if (credencial) {
          try {
            ...
            this.$store.dispatch("toast/showToast", {
              message: `Erro ao carregar o profissional`,
              color: "error",
            });
          } finally {
            this.$store.dispatch("loading/hideLoading");
